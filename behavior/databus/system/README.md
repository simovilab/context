...
sequenceDiagram
    ...
    API->>Backend: run_requested
    Backend->>RealtimeEngine: run_validation_requested
    alt RUN_VALIDATION_ACCEPTED
        RealtimeEngine->>Backend: run_accepted
        RealtimeEngine->>RealtimeEngine: run_initialized
    else RUN_VALIDATION_REJECTED
        RealtimeEngine->>Backend: run_rejected
    end
    RealtimeEngine->>API: run_notification_sent
    API->>Backend: todo gucci
```
...
- `register_run` must be processed sequentially.
